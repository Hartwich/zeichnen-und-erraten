...
function formatPhase(phase: string | undefined, en: boolean): string {
  switch (phase) {
    case "round_intro":
      return en ? "Round intro" : "Rundenvorbereitung";
    case "countdown":
      return "Countdown";
    case "playing":
      return en ? "Drawing" : "Zeichnen";
    case "locked":
      return en ? "Locked" : "Aufgeloest";
    case "result":
      return en ? "Result" : "Ergebnis";
    case "scoreboard":
      return en ? "Scoreboard" : "Punktestand";
    case "finished":
      return en ? "Round finished" : "Runde beendet";
    default:
      return en ? "Waiting" : "Warten";
  }
}

function buildReadyModel(context: ControllerGameRenderContext, en: boolean): ReadyLayoutModel | undefined {
  ...
  const label = en ? "Next Round" : "Naechste Runde";
...
export function buildZeichnenUndErratenControllerModel(
  ...
  return {
    ...
    title:
      ...
      (en ? "Draw & Guess" : "Zeichnen & Erraten"),
    subtitle: formatPhase(state.game?.phase, en),
    helperText:
      state.game?.message ?? (en ? "One player draws while the others guess." : "Ein Spieler zeichnet, die anderen raten."),
    language,
    disabled: state.game?.phase !== "playing",
    ready: buildReadyModel(context, en),
